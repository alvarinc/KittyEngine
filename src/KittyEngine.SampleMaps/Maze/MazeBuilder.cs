...
namespace KittyEngine.SampleMaps.Maze
{
    public class MazeBuilder
    {
        ...
        public List<VolumeDefinition> CreateMazeVolumes(MazeNode[,] nodes)
        {
            ...
            volumes.Add(MapHelper.CreateVolume(Colors.White, 0, 0, 0, sizeX * NodeSize, floorSize, sizeZ * NodeSize));
            for (int x = 0; x < sizeX; x++)
            {
                for (int z = 0; z < sizeZ; z++)
                {
                    volumes.AddRange(nodes[x, z].GetDefinition());
                }
            }
        ...
        public void DrawMaze(MazeNode[,] nodes, System.Drawing.Graphics gr, System.Drawing.Pen pen, double xScale, double zScale)
        {
            int sizeX = nodes.GetUpperBound(0) + 1;
            int sizeZ = nodes.GetUpperBound(1) + 1;
            for (int x = 0; x < sizeX; x++)
